...
/**
 * --------------------------------------------------------------------------
 * Game configuration — adjust these values to adapt to any Pick N + bonus
 * --------------------------------------------------------------------------
 */
$gameId            = 'RI2';
$leMainMin         = 1;
...
if (isset($stateName, $gName) && $stateName !== '' && $gName !== '') {
    ...
    $doc->setMetaData(
        'description',
        $stateName . ' ' . $gName . ' frequency analysis, draw history, active and quiet numbers, and full historical distribution. Analytical intelligence powered by LottoExpert.net.'
    );
}
...
$latestMainBalls = array_filter([$p1, $p2, $p3, $p4, $p5], static function ($v) { return $v !== ''; });
...
[$bonusCounts, $bonusLastSeenIndex] = leInitRange($leBonusMin, $leBonusMax);
...
$bonusChartLabels = leBuildNaturalLabels($leBonusMin, $leBonusMax);
$bonusChartValues = [];
...
if ($drawDate !== '') {
    ...
    if ($leHasBonus && $pb !== '') {
        ...
        $drawHistoryRows[] = [
            'label'    => $leBonusLabelShort . ' (' . lePad2($pb) . ')',
...
/**
 * --------------------------------------------------------------------------
 * URL helpers (game-specific routes preserved from target)
 * --------------------------------------------------------------------------
 */
$gIdEncoded       = rawurlencode((string) $gId);
$stateNameEncoded = rawurlencode((string) ($stateName ?? ''));
$gNameEncoded     = rawurlencode((string) ($gName ?? ''));
$sTnEncoded       = rawurlencode(strtolower((string) ($stateAbrev ?? '')));
...
$jsonLdData = [
    ...
    '@type'       => 'WebPage',
    'name'        => isset($stateName, $gName) ? $stateName . ' ' . $gName . ' Results Intelligence | LottoExpert.net' : 'Lottery Results Intelligence | LottoExpert.net',
    'description' => isset($stateName, $gName) ? $stateName . ' ' . $gName . ' draw frequency analysis, hot and cold numbers, recency tracking, and full historical data. Powered by LottoExpert.net.' : '',
    'url'         => $canonicalNoQuery,
];
$jsonLdOutput = json_encode($jsonLdData, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
?>
<script type="application/ld+json"><?php echo $jsonLdOutput; ?></script>
<style>
:root{
  --skai-blue:#1C66FF;
  --deep-navy:#0A1A33;
      ...
      <div class="skai-hero-top">
        ...
        <div class="skai-hero-copy">
          ...
          <div class="skai-hero-actions" aria-label="Primary actions">
            <a class="skai-btn skai-btn--primary" href="/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?gameId=RI2">
              Open SKAI Analysis
            </a>
            <a class="skai-btn skai-btn--secondary" href="/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=RI2">
              AI Predictions
            </a>
          ...
          <div class="skai-advanced-links" aria-label="Advanced analysis tools">
            <a class="skai-mini-link" href="/picking-winning-numbers/artificial-intelligence/skip-and-hit-analysis?game_id=RI2">Skip &amp; Hit Analysis</a>
            <a class="skai-mini-link" href="/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?game_id=RI2">MCMC Markov Analysis</a>
            <a class="skai-mini-link" href="/all-lottery-heatmaps?gId=<?php echo $gIdEncoded; ?>&amp;stateName=<?php echo $stateNameEncoded; ?>&amp;gName=<?php echo $gNameEncoded; ?>&amp;sTn=<?php echo $sTnEncoded; ?>">Heatmap Analysis</a>
          </div>
        </div>
  ...
  <section class="skai-strip" aria-label="Key takeaways">
    ...
    <article class="skai-stat">
      ...
      <div class="skai-stat-body">
        ...
        <div class="skai-stat-note">Main-number and <?php echo htmlspecialchars($leBonusLabelShort, ENT_QUOTES, 'UTF-8'); ?> draw windows currently loaded for this page view.</div>
      </div>
    </article>
  </section>
    ...
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Start with a clear high-level view. This layer is designed for fast orientation: which numbers are most active, which are quiet, and how the current draw relates to recent history. Main numbers range from <?php echo str_pad((string) $leMainMin, 2, '0', STR_PAD_LEFT); ?> to <?php echo str_pad((string) $leMainMax, 2, '0', STR_PAD_LEFT); ?>.
        </p>
      </div>
    </div>
              ...
              <?php foreach ($drawHistoryRows as $row) : ?>
                <div class="skai-history-item">
                  <div class="skai-history-name">
                    <?php if ($row['isBonus']) : ?>
                      <span class="skai-pill skai-pill--bonus" style="width:auto;padding:6px 10px;"><?php echo htmlspecialchars((string) $row['label'], ENT_QUOTES, 'UTF-8'); ?></span>
                    <?php else : ?>
                      <span class="skai-pill skai-pill--main"><?php echo htmlspecialchars((string) $row['label'], ENT_QUOTES, 'UTF-8'); ?></span>
                    <?php endif; ?>
                  </div>
  ...
  <section id="frequency-deep-dive" class="skai-section" aria-labelledby="frequency-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Move from summary to full reference. The first panel shows the complete main-number distribution across all values <?php echo str_pad((string) $leMainMin, 2, '0', STR_PAD_LEFT); ?>&ndash;<?php echo str_pad((string) $leMainMax, 2, '0', STR_PAD_LEFT); ?>. The second panel shows the <?php echo htmlspecialchars($leBonusLabelShort, ENT_QUOTES, 'UTF-8'); ?> distribution for the selected bonus window.
        </p>
      </div>
    </div>
      ...
      <div class="skai-two-col">
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--horizon">
            ...
            <span class="skai-card-sub">All values <?php echo str_pad((string) $leMainMin, 2, '0', STR_PAD_LEFT); ?>&ndash;<?php echo str_pad((string) $leMainMax, 2, '0', STR_PAD_LEFT); ?> across the last <?php echo (int) $nodCurrentMain; ?> drawings</span>
          </div>
          <div class="skai-card-body">
            <div class="skai-chart-shell">
        ...
        <div class="skai-grid">
          ...
          <div class="skai-card">
            <div class="skai-card-head skai-card-head--radiant">
              Full <?php echo htmlspecialchars($leBonusLabelShort, ENT_QUOTES, 'UTF-8'); ?> distribution
              <span class="skai-card-sub">All values <?php echo str_pad((string) $leBonusMin, 2, '0', STR_PAD_LEFT); ?>&ndash;<?php echo str_pad((string) $leBonusMax, 2, '0', STR_PAD_LEFT); ?> across the last <?php echo (int) $nodCurrentBonus; ?> drawings</span>
            </div>
            <div class="skai-card-body">
              <div class="skai-chart-shell">
                <div class="skai-chart-frame">
                  <canvas id="bonusChart" aria-label="<?php echo htmlspecialchars($leBonusLabelShort, ENT_QUOTES, 'UTF-8'); ?> distribution chart" role="img"></canvas>
                </div>
              </div>
            </div>
          </div>
  ...
  <section id="tables" class="skai-section" aria-labelledby="tables-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Use the tables for exact counts, recency, and personal tracking. Quick filters help narrow the view without losing full access to the complete dataset. Main numbers cover <?php echo str_pad((string) $leMainMin, 2, '0', STR_PAD_LEFT); ?>&ndash;<?php echo str_pad((string) $leMainMax, 2, '0', STR_PAD_LEFT); ?>; <?php echo htmlspecialchars($leBonusLabelShort, ENT_QUOTES, 'UTF-8'); ?> values cover <?php echo str_pad((string) $leBonusMin, 2, '0', STR_PAD_LEFT); ?>&ndash;<?php echo str_pad((string) $leBonusMax, 2, '0', STR_PAD_LEFT); ?>.
        </p>
      </div>
    </div>
      ...
      <form name="fqsearch" method="post" action="/all-us-lotteries/results-analysis?st=<?php echo htmlspecialchars((string) ($stateAbrev ?? ''), ENT_QUOTES, 'UTF-8'); ?>&amp;stn=<?php echo htmlspecialchars((string) ($stateName ?? ''), ENT_QUOTES, 'UTF-8'); ?>&amp;gm=<?php echo htmlspecialchars((string) ($gName ?? ''), ENT_QUOTES, 'UTF-8'); ?>#tables">
        <div class="skai-controls-row">
          <div class="skai-controls-left">
            ...
            <?php if ($leHasBonus) : ?>
            <label for="pnod"><?php echo htmlspecialchars($leBonusLabelShort, ENT_QUOTES, 'UTF-8'); ?> draw window</label>
    ...
    <div class="skai-section-body">
      <div class="skai-two-col">
        ...
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--horizon">
            Main numbers table
            <span class="skai-card-sub">Exact counts and recency for values <?php echo str_pad((string) $leMainMin, 2, '0', STR_PAD_LEFT); ?>&ndash;<?php echo str_pad((string) $leMainMax, 2, '0', STR_PAD_LEFT); ?></span>
          </div>
        ...
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--radiant">
            <?php echo htmlspecialchars($leBonusLabelShort, ENT_QUOTES, 'UTF-8'); ?> table
            <span class="skai-card-sub">Exact counts and recency for values <?php echo str_pad((string) $leBonusMin, 2, '0', STR_PAD_LEFT); ?>&ndash;<?php echo str_pad((string) $leBonusMax, 2, '0', STR_PAD_LEFT); ?></span>
          </div>

          <div class="skai-table-wrap">
            <table id="skai-bonus-table" class="skai-table" aria-label="<?php echo htmlspecialchars($leBonusLabelShort, ENT_QUOTES, 'UTF-8'); ?> frequency table">
              <thead>
                <tr>
                  <th>Number</th>
                  <th>Drawn Times</th>
              ...
              <tbody>
                <?php for ($i = $leBonusMin; $i <= $leBonusMax; $i++) : ?>
                  <?php
                  $number      = ($i < 10) ? '0' . $i : (string) $i;
                  ...
                  <tr>
                    ...
                    <td>
                      <input
                        ...
                        type="checkbox"
                        value="<?php echo htmlspecialchars($number, ENT_QUOTES, 'UTF-8'); ?>"
                        aria-label="Track <?php echo htmlspecialchars($leBonusLabelShort, ENT_QUOTES, 'UTF-8'); ?> number <?php echo htmlspecialchars($number, ENT_QUOTES, 'UTF-8'); ?>"
                      >
                    </td>
                  </tr>
                <?php endfor; ?>
          ...
          <div class="skai-tracked">
            <div class="skai-tracked-head">
              <h3 class="skai-tracked-title">Tracked <?php echo htmlspecialchars($leBonusLabelShort, ENT_QUOTES, 'UTF-8'); ?> values</h3>
              <div class="skai-tracked-actions">
          ...
          <div class="skai-note">
            Tracking is local to this page view. It is intended as a lightweight comparison aid while you move between the overview, tables, and advanced analysis tools.
          </div>
        </div>
        <?php endif; ?>
      ...
      <div class="skai-tool-grid">
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?gameId=RI2">Open SKAI Analysis</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=RI2">Open AI Predictions</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/skip-and-hit-analysis?game_id=RI2">Open Skip &amp; Hit</a>
          </div>
        </article>
      </div>

      <div class="skai-utility-grid">
        <a class="skai-utility-link" href="/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?game_id=RI2">MCMC Markov Analysis</a>
        <a class="skai-utility-link" href="/all-lottery-heatmaps?gId=<?php echo $gIdEncoded; ?>&amp;stateName=<?php echo $stateNameEncoded; ?>&amp;gName=<?php echo $gNameEncoded; ?>&amp;sTn=<?php echo $sTnEncoded; ?>">Heatmap Analysis</a>
        <a class="skai-utility-link" href="/lottery-archives?gId=<?php echo $gIdEncoded; ?>&amp;gName=<?php echo $gNameEncoded; ?>&amp;stateName=<?php echo $stateNameEncoded; ?>&amp;sTn=<?php echo $sTnEncoded; ?>">Lottery Archives</a>
        <a class="skai-utility-link" href="/lowest-drawn-number-analysis?gId=<?php echo $gIdEncoded; ?>&amp;stateName=<?php echo $stateNameEncoded; ?>&amp;gName=<?php echo $gNameEncoded; ?>&amp;sTn=<?php echo $sTnEncoded; ?>">Lowest Number Analysis</a>
      </div>
    </div>
  </section>
  ...
  <section class="skai-section" aria-labelledby="method-title">
    ...
    <div class="skai-section-body">
      <div class="skai-method-note">
        <strong>Interpretation guidance:</strong> Frequency, recency, and spacing can provide useful context for reviewing draw history, but they should be treated as descriptive signals rather than guarantees. The purpose of this page is to make the recent behavior of <?php echo htmlspecialchars((string) ($gName ?? 'this game'), ENT_QUOTES, 'UTF-8'); ?> easier to understand, compare, and carry into deeper analysis. Main numbers (<?php echo str_pad((string) $leMainMin, 2, '0', STR_PAD_LEFT); ?>&ndash;<?php echo str_pad((string) $leMainMax, 2, '0', STR_PAD_LEFT); ?>) and <?php echo htmlspecialchars($leBonusLabelShort, ENT_QUOTES, 'UTF-8'); ?> values (<?php echo str_pad((string) $leBonusMin, 2, '0', STR_PAD_LEFT); ?>&ndash;<?php echo str_pad((string) $leBonusMax, 2, '0', STR_PAD_LEFT); ?>) are tracked and presented separately to ensure analytical accuracy.
      </div>
    </div>
  </section>
...
(function () {
  ...
  var chartData = {
    ...
    hasBonus: <?php echo $leHasBonus ? 'true' : 'false'; ?>
  };
  ...
  function renderCharts() {
    ...
    var bonusCanvas     = safeGetCanvas('bonusChart');
    ...
    if (bonusCanvas && chartData.hasBonus && chartData.bonusLabels && chartData.bonusLabels.length) {
